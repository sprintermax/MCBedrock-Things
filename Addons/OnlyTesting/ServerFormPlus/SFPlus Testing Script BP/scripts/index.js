...
const customModalFormTest = new ModalFormData()
    ...
    .textField(
        "Custom Modal Form Text Field 1",
        "Custom Modal Form Text Field 1 PlaceHolder"
    )
...
world.beforeEvents.itemUse.subscribe(({ itemStack, source }) => {
    ...
    system.run(async () => {
        if (itemStack.typeId === "minecraft:stick")
            vanillaLongForm.title("Vanilla Long Form Title").show(source);
        if (itemStack.typeId === "minecraft:emerald")
            vanillaModalForm.title("Vanilla Modal Form Title").show(source);
        if (itemStack.typeId === "minecraft:diamond")
            vanillaMessageForm.title("Vanilla Message Form Title").show(source);

        if (itemStack.typeId === "minecraft:blaze_rod")
            customLongFormTest.title("Custom Long Form Title").show(source);
        if (itemStack.typeId === "minecraft:bone")
            customModalFormTest.title("Custom Modal Form Title").show(source);
    ...
    // if (itemStack.typeId === "minecraft:stick" && itemStack.nameTag === "Open Form 1") {
    //     // Form
    // };
});

...
function td(d, mde) {
	fs.readdirSync(d).forEach(File => {
		...
		if (fs.statSync(abs).isDirectory()) return td(abs);
			...
			const dt = JSON.parse(fs.readFileSync(abs).toString("UTF-8").replace(/[\n\r\t]/g, ""))
			return fs.writeFileSync(abs, JSON.stringify(Wj(dt, mde)).replace(/\\\\u/g, '\\u'));
		}
	});
}
...
function Cvt(str) {
	...
	for (let i = 0, strLen = str.length; i < strLen; i++) nstr.push('\\u' + parseInt(str.charCodeAt(i), 10).toString(16).padStart(4, '0'));
	return nstr.join('');
}
